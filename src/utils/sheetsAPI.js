...
export const updateRowInSheet = async (tableName, payload, appId) => {
  try {
    ...
    // Dùng YYYY-MM-DD (ISO) khi ghi để dứt điểm lỗi nhảy ngày sang 2027
    ...
    const response = await fetch(getApiUrl(appId, tableName), {
      ...
      body: JSON.stringify({
        Action: "Edit",
        Properties: {
          Locale: "en-US", // Dùng en-US để AppSheet hiểu chuẩn ISO YYYY-MM-DD
          Timezone: "Asia/Ho_Chi_Minh",
        },

...
export const normalizeKey = (str) => {
    ...
    if (sClean.includes('nguoi') || sClean.includes('user') || sClean.includes('nguoichi')) return 'nguoiCapNhat';
...
export const updateRowInSheet = async (tableName, payload, appId) => {
  try {
    ...
    if (normTableName === "giaodich" || normTableName === normGiaoDichEnv) {
      ...
      formattedPayload[getBestColumnName(tableName, 'nguoiCapNhat', ['Người cập nhật', 'Người chi', 'nguoiCapNhat'])] = payload.nguoiCapNhat || "Ba";
    }
...
export const addRowToSheet = async (tableName, payload, appId) => {
  try {
    ...
    if (normTableName === "giaodich" || normTableName === normGiaoDichEnv) {
      ...
      formattedPayload[getBestColumnName(tableName, 'nguoiCapNhat', ['Người cập nhật', 'Người chi', 'nguoiCapNhat', 'User'])] = payload.nguoiCapNhat || "Ba";
    }

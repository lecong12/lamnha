...
export const updateRowInSheet = async (tableName, payload, appId) => {
  try {
    ...
    // Gửi định dạng YYYY-MM-DD (ISO) là cách an toàn duy nhất để không bị đảo Ngày/Tháng
    ...
    const response = await fetch(getApiUrl(appId, tableName), {
      ...
      body: JSON.stringify({
        Action: "Edit",
        Properties: {
          Locale: "en-US", // BẮT BUỘC dùng en-US khi ghi ISO để tránh lỗi 10/3 -> 3/10
          Timezone: "Asia/Ho_Chi_Minh",
        },
...
export const addRowToSheet = async (tableName, payload, appId) => {
  try {
    ...
    // Luôn gửi định dạng ISO YYYY-MM-DD
    ...
    const response = await fetch(getApiUrl(appId, tableName), {
      ...
      body: JSON.stringify({
        Action: "Add",
        Properties: {
          Locale: "en-US", // BẮT BUỘC dùng en-US khi ghi ISO
          Timezone: "Asia/Ho_Chi_Minh",
        },
        Rows: [formattedPayload],
      }),
